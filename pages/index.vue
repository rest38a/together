<template >
    <div >
        <Header />
        <Action />
        <WinCoupon v-if="!mobile" />
        <MobileWinCoupon v-else />
        <Podarok v-if="!mobile" />
        <MobilePodarok v-else />
        <Faq />
        <Footer />
        <Offcanvas />
    </div >
...
<script >
    ...
    export default {
        ...
        head: {
            title: 'Мы Вместе Иркутск We are together',
            titleTemplate: 'Мы Вместе Иркутск We are together Иркутск',
            meta: [
                { charset: 'utf-8' },
